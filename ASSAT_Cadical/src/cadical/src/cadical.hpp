...
public:
  ...
  //
  void phase (int lit);
  void initphase (int lit);
  void phase_target(int lit,int value);
  ...
  // void read_dimacs (char *in, int max_var, bool incremental,std::vector<int> cube_literals);

  // void read_dimacs (char *in);

  // const char *read_dimacs (const char *path, int &vars, int strict = 1);

void read_dimacs (char *in,int &max_var);
