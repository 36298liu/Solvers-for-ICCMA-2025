 ...
 int Internal::cdcl_loop_with_inprocessing () {
  ...
  while (!res) {
    if (unsat)
      ...
      res = 20;
    else if (!propagate ())//单子句传播有冲突
       analyze (); // propagate and analyze
    else if (iterating)//处理某些特殊学习策略或外部证明步骤，比如：在固定变量上反复加 unit 子句 || DRUP/DRAT proof logging
      ...
      if (!external_check_solution () || unsat) {
        ...
        res = 10;
    } else if (search_limits_hit ()) //达到设定的冲突次数 / 决策次数等限制，跳出循环
      break;                               // decision or conflict limit
    else if (terminated_asynchronously ()) // externally terminated //外部终止
      break;
    else if (restarting ())
