...
void Solver::phase_target (int lit,int value) {
  external->phase_target (lit,value);
}

void Solver::initphase (int lit) {
  external->initphase (lit);
}
...
// void Solver::read_dimacs(char *in, int &max_var) {
...
//       // 解析头部信息 p cnf num_vars num_clauses
//       if (!header_parsed && token == "p") {
//           string format;
//           iss >> format >> num_vars >> num_clauses;
//           if (format != "cnf") {
//               cerr << "错误: 仅支持标准 CNF 格式" << endl;
//               return;
//           }
//           max_var = num_vars;  // 存储最大变元编号
//           header_parsed = true;
//           continue;
//       }
...
//       while (clause_stream >> literal) {
//           this->add(literal);  // 使用 CaDiCaL 添加字面量
//           if (literal == 0) break;  // 0 结束当前子句
//       }
//   }

//   int in_vars = max_var/3;
...
void Solver::read_dimacs(char *in, int &max_var) {
  ...
  while (input >> token) {
      ...
      if (!header_parsed && token == "p") {
          string format;
          input >> format >> num_vars >> num_clauses;
          if (format != "cnf") {
              cerr << "错误: 仅支持标准 CNF 格式" << endl;
              return;
          }
