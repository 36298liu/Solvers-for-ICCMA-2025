...
bool PreferredSemantics::compute(Argument *arg, bool firstonly)
{
	...
	do
	{
		...
		do
		{
			...
			if (res.undecargs()->empty())//if undecargs is empty ,break
			{
				break;
			}
		...
		{
			this->labellings.push_back(prefcand);//outcome is preferred labelling
		}
		...
		//if undecargs is empty ,following steps
		OrClause oppsolution = OrClause();
		SetArgumentsIterator iter;
